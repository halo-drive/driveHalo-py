...
def init_camera(camera_type="usb"):
    """Initialize camera with support for both USB and GMSL

    Args:
        camera_type (str): Type of camera to initialize ("usb" or "gmsl")

    Returns:
        cv2.VideoCapture: Initialized camera capture object
    """
    logger.info(f"Initializing {camera_type.upper()} camera...")

    if camera_type.lower() == "gmsl":
        # GMSL camera pipeline for NVIDIA platforms
        # Assumes nvarguscamerasrc is available and GMSL camera is properly connected
        gst_pipeline = (
            "nvarguscamerasrc sensor-mode=0 sensor-id=0 ! "  # First camera group, primary sensor
            "video/x-raw(memory:NVMM), "
            "width=1920, height=1080, "  # GMSL typically supports higher resolutions
            "format=NV12, framerate=30/1 ! "
            "nvvidconv ! "  # Convert to CPU-accessible format
            "video/x-raw, format=BGRx ! "
            "videoconvert ! "  # Final conversion to BGR
            "video/x-raw, format=BGR ! "
            "appsink max-buffers=1 drop=true"
        )
    else:
        # Original USB camera pipeline
        gst_pipeline = (
    ...
    if not cap.isOpened():
        raise RuntimeError(f"Failed to initialize {camera_type.upper()} camera with GStreamer pipeline")
...
def main():
    ...
    # Initialize with GMSL camera
    # You can change this to "usb" for USB camera
    cap = init_camera(camera_type="gmsl")
    ...
    try:
        ...
        while True:
            ...
            # If using GMSL camera, resize frame to match model input
            # GMSL typically outputs higher resolution
